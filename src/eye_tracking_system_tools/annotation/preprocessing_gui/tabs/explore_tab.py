"""Data Exploration tab — full-block traces + synced video (Phase 0–1)."""
...
class ExploreTab(BaseTab):
    ...
    def __init__(self, state, config, parent=None):
        ...
        self._video: ExploreVideoPanel | None = None
    ...
    def build_ui(self) -> None:
        ...
        self._info = QtWidgets.QLabel(
            "Load a block with final_sync_df.csv, then use Load prev analysis "
            "to explore synchronized traces and video."
        )
        ...
        left_lay.addWidget(self._plot, stretch=1)
        self._time_label = QtWidgets.QLabel("t = — ms")
        left_lay.addWidget(self._time_label)
        splitter.addWidget(left)

        self._video = ExploreVideoPanel()
    ...
    def _set_time_label(self, ms: float) -> None:
        if self._time_label is not None:
            self._time_label.setText(f"t = {ms:.1f} ms")

    def _on_plot_time_selected(self, ms: float) -> None:
        self._set_time_label(ms)
        if self._syncing_time or self._video is None:
            return
    ...
    def _reload_videos(self, le_df, re_df) -> None:
        if self._block is None or self._video is None:
            return
        out = self._state.output_folder
        if out is None:
            out = self._block.analysis_path
        try:
            self._video.load_block(
                self._block.block_path,
                out,
                animal_call=self._block.animal_call,
                experiment_date=self._block.experiment_date,
                block_num=self._block.block_num,
                le_ellipse_df=le_df,
                re_ellipse_df=re_df,
            )
        except Exception as exc:
