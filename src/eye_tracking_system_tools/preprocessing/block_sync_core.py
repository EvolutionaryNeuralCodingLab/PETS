...
def _delta_analysis(t_sec: np.ndarray, label: str, cov_warn: float = 0.05) -> dict:
    ...
    print(
        f"[{label}] frames={len(t_sec):,} | median fps={fps:.3f} | "
        f"CoV(dt)={cov*100:.2f}% | outliers(+/-1-99%)={out_frac*100:.2f}%"
    )
    if cov > cov_warn:
        ...
        for k in range(n_events):
            ...
            if verbose and n_corrected_this_event > 0:
                ...
                print(
                    ...
                    f"dt_start={dt_start*1000:.1f}ms, dt_end={dt_end*1000:.1f}ms "
...
def build_final_sync_df_merge_nearest(
    ...
    if export_csv:
        ...
        if verbose:
            print(f"[OK] Saved final sync CSV -> {outp}")
    block.final_sync_df = final_df

    if verbose:
        print(
            f"[INFO] Grid rows: {len(grid):,} | tick ~= {tick_ms:.3f} ms | "
            f"tol={tol} samp (tol_frac={tol_frac:.2f})"
        )
...
def create_distance_plot(distances, top_dist_to_show=500):
    ...
    if sns is None:
        raise ImportError("seaborn is required for create_distance_plot()")
